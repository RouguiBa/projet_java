...
public class MapPanel extends JPanel {
	...
	public Point computePosition(PointInteret pointInteret) {
		/** Cette méthode a été modifiée et dédoublée afin qu'elle corresponde aux différents types de
		 * points rencontrés lors du code. 
		 * 
		 */
		int x = lon2position(pointInteret.x, getZoom());
		int y = lat2position(pointInteret.y, getZoom());
		return new Point(x, y);
	}
	public Point computePosition1(Point.Double point) {
		int x = lon2position(point.x, getZoom());
		int y = lat2position(point.y, getZoom());
		return new Point(x, y);
	}
	public Point computePosition2(Point point) {
		int x = lon2position(point.x, getZoom());
		int y = lat2position(point.y, getZoom());
		return new Point(x, y);
	}
	...
	protected void paintComponent(Graphics gOrig) {
		/** La methode paintComponent à été modifiée de sorte que la fonction d'affichage paintAppliInstruction(g)
		 * ne se compile pas si l'object scene est null (i.e. elle ne contient pas d'object)
		 */
	...
	public Point coord(String texte) {
		
		/** La méthode coord permet de transformer les textes contenant les coordonnées écran
		 * en un point de coordonnées x,y. 
		 *@author	josephine
		 *@param	String des coordonnées écran
		 *@return	Point des coordonnées
		 */
...
public Point vecTranslation(Point point) {
	
		/** La méthode vecTranslation(Point point) permet de transformer les coordonnées 
		 * cartographiques du point d'intéret en coordonnées écran.
		 *@author	josephine
		 *@param	Point en coordonnées cartographiques
		 *@return	Point en coordonnées écrans
		 */
		
		Scene scene = Scene.getInstance();
		Point centre_carto = scene.getCentreCarto();
		int x_centre_screen = scene.getWidth()/2;
		int y_centre_screen = scene.getHeight()/2;
		Point vectTranslation = new Point(x_centre_screen - centre_carto.x, y_centre_screen - centre_carto.y);
		
		int x = (int) point.x + vectTranslation.x;
		int y = (int) point.y + vectTranslation.y;
...
private void paintAppliInstruction(Graphics2D g2d) {
	...
	/** La méthode paintAppliInstruction(Graphics2D g2d) permet de tracer l'itinéraire ainsi 
	 * que les points d'interêts
	 * @author josephine
	...
	Point start = coord(scene.getStart());  
	Point end = coord(scene.getEnd());
	
	ArrayList<PointInteret> listePoints = scene.getCandidat();
	ArrayList<double[]> ptDecision = scene.getPtDecision();
			
	//Affichage graphique: 
	
	g2d.setPaint(Color.red); 
	g2d.fillRoundRect(start.x, start.y,3,3,3,3);

	
	// Affichage point de décision 
	g2d.setPaint(Color.black);
    for (int j=0; j< ptDecision.size(); j++) {
    	        	
    	// Transformation des coordonnées géo=> carto du point j 
    	Point coord_c = computePosition1(new Point.Double (ptDecision.get(j)[0], ptDecision.get(j)[1]));
    	
    	// Transformation des coordonnées carto => screen du point j 
    	Point point_s = new Point(vecTranslation(coord_c).x,vecTranslation(coord_c).y);
    	
        g2d.fillRoundRect(point_s.x, point_s.y, 5,5,5,5);
       
    	
    	if (j ==0) {
	        //je trace la ligne entre le point de départ et le point de décision
	        g2d.drawLine(start.x, start.y, point_s.x,point_s.y);
    	} 
	        
	     if (j == (ptDecision.size() -1)) {
 	        //je trace la ligne entre le point de décision et le point d'arrivée 
    	      g2d.drawLine(point_s.x, point_s.y, end.x, end.y);
        	}
	     
   	     if(0<j && j<=(ptDecision.size()-1 )) {
     	     	        
 	        // Transformation des coordonnées du point i-1
         	Point coordi_carto = computePosition1(new Point.Double (ptDecision.get(j-1)[0], ptDecision.get(j-1)[1]));

         	Point pointi_s = new Point( vecTranslation(coordi_carto).x, vecTranslation(coordi_carto).y);
         	
         	//Affichage du point i-1
 	        g2d.fillRoundRect(pointi_s.x,pointi_s.y, 5,5,5,5);

 	        g2d.drawLine(pointi_s.x, pointi_s.y,point_s.x, point_s.y);
 	        
     	}
    }
    
    g2d.setPaint(Color.red); 
    g2d.fillRoundRect(end.x, end.y,3,3,3,3);
    
    // Affichage pt interets  : 
    g2d.setPaint(Color.blue);
    
    	for (int i= 0; i<listePoints.size(); i++) {
    		
    	//Transformation des coordonnées
    	Point coord_carto = computePosition1(new Point.Double (listePoints.get(i).x, listePoints.get(i).y));
    	
    	int x = vecTranslation(coord_carto).x;
    	int y = vecTranslation(coord_carto).y;
    	
    	Point pt_screen = new Point(x,y);

    	g2d.fillRoundRect(pt_screen.x, pt_screen.y, 5,5,5,5);
    	}
